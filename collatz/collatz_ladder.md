...
scores `7--9.4` give progressively sharper uniform stopping-time bounds, and
...
| **0.3** | **[The Weakest Collatz-like Conjecture](weakest_collatzlike_conjecture.md).**  Some signed unit map and some $u$ coprime to the map base satisfy $D_K(u^k)=\omega(\log K)$ for every sufficiently large fixed exponent $k$. |
| **0.8** | **[All-Start Signed Superlogarithmic Contraction](signed_superlogarithmic_contraction.md).**  Some signed unit map satisfies $D_K(n)=\omega(\log K)$ for every $n\in\mathbb N_0$. |
| **1.2** | **[All-Start Nonnegative Superlogarithmic Contraction](nonnegative_superlogarithmic_contraction.md).**  The preceding conclusion has a witness with positive multipliers and nonnegative offsets. |
| **1.5** | **Pure-division superlogarithmic contraction.**  The all-start conclusion has a witness whose contracting branches are pure division. |
| **1.8** | **Pure-division polylogarithmic contraction.**  Such a map has $D_K(n)\ge(\log K)^{1+\delta}$ eventually for every start, for some fixed $\delta\gt0$. |
| **2.4** | **Pure-division power contraction.**  Such a map has $D_K(n)\ge K^\delta$ eventually for every start, for some fixed $\delta\gt0$. |
| **3.0** | **[Positive Division-Density Conjecture](positive_division_density.md).**  Some pure-division map has $\liminf_{K\to\infty}D_K(n)/K\gt0$ for every start. |
| **3.6** | **Classical positive division density.**  Every positive shortcut-Collatz orbit has positive lower even-step density. |
| **4.3** | **Classical drift-threshold frequency.**  Every positive shortcut-Collatz orbit satisfies $\liminf_{K\to\infty}D_K^{\rm Col}(n)/K\ge\theta_{\rm Col}$, where $\theta_{\rm Col}=1-\log2/\log3$. |
| **5.0** | **Classical bounded-orbit conjecture.**  Every positive shortcut-Collatz orbit is bounded, equivalently eventually periodic in some positive cycle. |
| **6.0** | **Classical Collatz conjecture.**  Every positive integer reaches $1$, equivalently the cycle $1\leftrightarrow2$. |
| **7.0** | **Polynomial uniform stopping bound.**  There is an $A\gt0$ such that $M(X)=O(X^A)$. |
| **8.0** | **Polylogarithmic uniform stopping bound.**  There is an $A\gt0$ such that $M(X)=O((\log X)^A)$. |
| **9.0** | **Linear-log uniform stopping bound.**  $M(X)=O(\log X)$. |
| **9.4** | **Classical stopping-time limiting constant.**  $M(X)/\log X$ converges to a finite positive limit $c_{\rm Col}$. |
| **9.5** | **Stopping-time limit plus [universal one-expander periodicity](one_expander_universal_periodicity.md).**  The score-`9.4` statement holds, and every map in the restricted positive, coprime, pure-division, one-expander class with $a\lt B^B$ has every nonnegative orbit eventually periodic. |
...
which expanding branches occur.  The score-`4.3` inequality is deliberately
...
for $M(X)$ implies classical Collatz.  Scores `7`, `8`, and `9` successively
ask for polynomial, polylogarithmic, and logarithmic bounds.  Score `9.4` asks
...
$3^o/2^{o+e}\lt1$.  Hence $e/(o+e)\gt\theta_{\rm Col}$, proving that score `5`
implies score `4.3`.  Classical Collatz implies boundedness.  Each uniform
stopping-time bound implies Collatz and the successively sharper bounds imply
one another.

The score-`9.5` endpoint is a conjunction.  It retains the score-`9.4`
...
| Score | Central $C_{Col}(s)/B_{Col}$ | $\log_{10}C_{Col}(s)$ at the central stock |
|---:|---:|---:|
| 0 | 1.00 | 3.30 |
| 0.3 | 1.07 | 3.33 |
| 0.8 | 1.19 | 3.38 |
...
Thus score `5` is centrally about `1.6` doublings over the area-specific 2026
stock.  The one-expander endpoint has a central cross-problem placement near
...
Using $C_{Col}(s)=B_{Col}+F_{Col}(s)$ and the cumulative `1.1` rule gives these
provisional probabilities, each uncertain by at least `15--20` percentage
points:

| Transition | Skip probability | Main reason it could be skipped |
|---:|---:|---|
| **0 to 1** | **20%** | A universal rigidity lemma may solve an all-start statement before a ray-specific construction is isolated. |
| **1 to 2** | **30%** | A lower-bound mechanism may naturally yield positive density rather than a chosen intermediate rate. |
| **2 to 3** | **45%** | Quantitative branch control may arrive with a substantial explicit constant. |
| **3 to 4** | **40%** | A classical parity-distribution theorem may reach the drift threshold as soon as it proves positive density. |
| **4 to 5** | **30%** | A direct descent argument may establish boundedness without separately isolating frequency control. |
| **5 to 6** | **35%** | A universal Collatz mechanism may prove recurrence and cycle identification together. |
| **6 to 7** | **25%** | A constructive Collatz proof may carry an explicit polynomial stopping bound. |
| **7 to 8** | **35%** | A quantitative contraction proof may naturally be polylogarithmic. |
| **8 to 9** | **30%** | Extreme-value control may sharpen a polylogarithm directly to logarithmic order. |
...
spacing.  Historical fame does not raise a weak theorem's score.  Thus `6` is not
