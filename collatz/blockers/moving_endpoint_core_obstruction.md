...
gives constants `A>1` and `C` such that
...
target fails.  Then there are constants `\delta,\eta>0` and arbitrarily large
`M` for which
...
```math
\log\tau_M-\log\tau_1
=\sum_{m<M}\log\frac{\tau_{m+1}}{\tau_m}.
```
...
Let `j<k` be marked indices.  Eliminating the block recurrences between their
...
For two marked indices `j<k`, the macro link specializes to
...
starting integer `n`, and constants `\delta>0` and `R\ge1`.  Then there should
exist a bound
...
for every `i`, where `q>Q`.

Equivalently, for every fixed orbit and every `\delta>0`, the proportional-gap
