...
| Collatz | `collatz/weakest_collatzlike_conjecture.md`: one signed unit map and exact scaled ray `d u^k`, infinitely many `k`, with `u` multiplicatively independent of `B`, pointwise `D_K = ω(log K)` | `Collatz.WeakestScaledRay` | Exact. The map and `d,u` are fixed before the infinite set of good exponents; each good `k` is fixed before the `K→∞` limit. |
