...
| Twin prime | `twin-prime/difficulty_ladder.md`, score 0.3: `H₁≤244` or `EH[1/2+δ]` for some fixed `δ>0` | `PrimeGaps.WeakestPrimePattern` | Gap branch exact. EH branch is a typed analytic schema retaining the standard-discrepancy obligation, `X^θ` cutoff, all-log-power error bounds, and OR quantifier. |
